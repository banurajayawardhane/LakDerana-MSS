...
if (isset($_POST['submit']))
{
	...
	if($query->num_rows > 0)
    {
        ...
        if($status == 'Time_in')
        {
            if($time_now > 11)
            {
                $sql = "SELECT * FROM attendance WHERE `user_id` = '$id' AND `date` = '$today_date' AND time_in IS NOT NULL";
                $query = $conn->query($sql);
                if($query->num_rows > 0)
                {
                ...
                else
                {
                    ...
                    $sql = "INSERT INTO attendance (`user_id`,`status`,`time_in`,`date`) VALUES ('$id', 'late', NOW(), '$today_date')";
            ...
            else
            {
                ...
                {
                    $sql = "INSERT INTO attendance (`user_id`,`status`,`time_in`,`date`) VALUES ('$id', 'present', NOW(), '$today_date')";
                    $result = mysqli_query($conn, $sql);
                    if ($result)
                    {
        ...
        {
            $sql = "SELECT * FROM attendance WHERE `user_id` = '$id' AND `date` = '$today_date' AND time_out IS NOT NULL";
			$query = $conn->query($sql);
			if($query->num_rows > 0)
            {
            ...
            {
                $sql = "UPDATE attendance SET time_out= '$time_now' WHERE `user_id`='$id' AND `date`='$today_date'";
			    $result = mysqli_query($conn, $sql);
                if ($result)
                {
...
<head>
	...
	<title>Mark attendance - LakDerana Hotels</title>
</head>
